...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Ubah Tahun</h6>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('adminAchievementYearEditMethod', $year->id) }}">
                @method('PUT')
                @csrf
                <div class="mb-3">
                    <label class="form-label">Tahun</label>
                    <input type="number" class="form-control" name="tahun" value="{{ $year->nama }}" placeholder="Tahun ..." required>
